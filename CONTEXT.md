...
**Trigger**:
An ephemeral occurrence created for each event-delivery and ProjectAutomationRule match that attempts to start or resume an AgentWork with the rule's contextual instruction. It retains the triggering artifact references internally to pre-scope the Sandbox and tools, while the Agent discovers current artifact state through those tools rather than receiving an event snapshot. One AgentWork admits at most one running Trigger; a contender fails immediately rather than waiting, while Triggers for distinct AgentWorks may run concurrently without ordering. An admitted Trigger becomes completed when the Agent runtime returns normally or failed when a technical interruption prevents that return; either outcome leaves the permanent AgentWork unchanged. Deliveries are neither deduplicated nor coalesced, and repeated executions are runtime behavior rather than durable domain objects.
