...
An admitted Trigger retains the structured event context, contextual instruction, and capability snapshot with which it began. Later events or configuration changes are not injected into it and do not alter or cancel it. The Agent may observe newer artifact state only by reading it through its tools. A later event is evaluated normally and, if it resolves to the occupied AgentWork, its distinct Trigger fails under the same admission rule.
