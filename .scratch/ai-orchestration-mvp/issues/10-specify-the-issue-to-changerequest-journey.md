...
Each matching rule creates or resumes its own AgentWork according to its continuity policy. Yapir supplies the rule's contextual instruction, pre-scoped tools for the active Issue, and the ProjectAgentConfiguration's capability snapshot; the Agent fetches current Issue state through those tools rather than receiving an injected event snapshot. Yapir imposes no durable planning, coding, submission, or handoff phases. The Agent decides how to perform the implementation within those boundaries. An execution that does not produce the expected result adds no special business state and does not complete or otherwise change the permanent AgentWork.

The implementing Agent uses one stable AgentWork branch throughout the journey and may check out, commit, and push as often as needed. A User with sufficient GitHub access may also push to that branch. The branch is a correlation convention and working location, not a security boundary; GitHub permissions and branch protection remain authoritative. Before a ChangeRequest exists, only application of the configured GitHub Issue label can activate an MVP rule; repository pushes and other Issue activity do not produce automation events.
...
Successful submission has no additional Yapir readiness state, reviewer assignment, or special Agent-to-Agent handoff. The newly known ChangeRequest is immediately available in Yapir. Its subsequent GitHub `pull_request.opened` webhook produces the ordinary `ChangeRequest created` event that every matching ProjectAutomationRule may handle independently; the submission action itself does not emit that event.
