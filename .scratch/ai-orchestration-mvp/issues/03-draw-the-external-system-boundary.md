...
Yapir is canonical for Agent configuration, automation configuration, AgentWork continuity, CommentSets, Comments, replies, and resolutions. Every CommentSet is tied to the current commit when submitted; once the ChangeRequest head differs, that CommentSet is considered old. A later CommentSet can cover the newer commit and can continue or resolve earlier Yapir discussions.
