...
Agents interact with external artifacts through explicit generic tools backed by scoped Yapir APIs. The available tools define the capabilities for each use case; this boundary does not establish one universal action catalogue. Yapir validates each request and performs provider API writes through the GitConnector, so an Agent never receives the connector's durable API credential.
